...
def validate():
    ...
    with torch.set_grad_enabled(False):
        for i_batch, batch in enumerate(iterator_va):
            ...
            gloss_v = torch.mean(torch.abs(netVD(gen_voc)-gen_voc))

            # ### Train discriminators ###
            real_dloss_v = torch.mean(torch.abs(netVD(voc)-voc))
            fake_dloss_v = torch.mean(torch.abs(netVD(gen_voc.detach())-gen_voc.detach()))
...
class NetG(nn.Module):
    def __init__(self, feat_dim, z_dim):
        ...
        blocks = [
            ...
            nn.LeakyReLU(),
            G3Block(mfd, ks, dilation=2, groups=groups, num_folds=num_folds),
            G3Block(mfd, ks, dilation=4, groups=groups, num_folds=num_folds),
        ]
...
class NetVD(nn.Module):
    def __init__(self, input_size):
        ...
        # ### Main body ###
        blocks = [
            nn.Conv1d(input_size, mfd, 3, 1, 1),
            nn.GroupNorm(groups, mfd),
            nn.LeakyReLU(),
...
if __name__ == '__main__':
    ...
    # Dirs and fps
    save_dir = os.path.join(base_out_dir, 'save.free_singer')
