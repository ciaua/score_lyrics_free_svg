...
class TrainingManager(object):
    ...
    def save_resuming(self):
        # Put a file indicating the current stage
        status_fp = os.path.join(self.output_dir, 'status.txt')
        append_line(status_fp, '<Re-Start>\n')

        # Copy script
        if self.script_path is not None:
            out_script_fp = os.path.join(
                self.output_dir, 'training_script.resumed.py')
            shutil.copy(self.script_path, out_script_fp)
    ...
    def save_final(self):
        # Save record
        record_fp = os.path.join(self.record_dir, 'record.final.json')
        write_json(record_fp, self.records)

        # Save params
        params_fp_list = [
            os.path.join(self.model_dir, 'params.{}.final.torch'.format(name))
            for name in self.names
        ]
    ...
    def get_resumed_epoch(self, param_dir, model_name):
        fns = [fn for fn in os.listdir(param_dir) if fn.startswith(f'params.{model_name}.@')]
        resumed_epoch = max([int(fn.split('.')[2].replace('@', '')) for fn in fns])
        return resumed_epoch

    def resume_training(self, resumed_model_id, resumed_save_dir, metric):
        '''
        resumed_model_id:
        ...
        status_fp = os.path.join(self.output_dir, 'status.txt')
        resumed_epoch = max(
            [int(line.replace('Epoch: ', ''))
             for line in read_lines(status_fp) if line.startswith('Epoch: ')])
        '''
        resumed_epoch = self.get_resumed_epoch(param_dir, self.names[0])

        # Copy the old model dir to the new model dir
        # copy_tree(r_base_model_dir, self.output_dir)

        # Resume networks and optimizers

        for name, network, optimizer in zip(self.names, self.networks, self.optimizers):
            param_fp = os.path.join(param_dir, 'params.{}.@{}.torch'.format(name, resumed_epoch))
            load_model(param_fp, network, optimizer)

        # Set best loss and best score
        record_fp = os.path.join(base_model_dir, 'record', 'record.@{}.json'.format(resumed_epoch))
        record = read_json(record_fp)

        self.best_va_metrics = record[str(resumed_epoch)]['record']['best_va_metrics']
